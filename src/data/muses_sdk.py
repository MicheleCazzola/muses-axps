"""
    SDK utilities for loading and processing MUSES dataset.
    This module includes functions for loading calibration data, projecting LiDAR point clouds to image space, motion compensation of point clouds, and saving processed data.

    Source: https://github.com/timbroed/MUSES
    Credits: Tim Broedermann, David Bruggemann, Christos Sakaridis, Kevin Ta, Odysseas Liagouris, Jason Corkill, and Luc Van Gool.
"""
...
def read_gnss_file(load_path):
    ...
    """
    keys = [
        'iTOW', 'timestamp', 'tAcc', 'lon', 'lat', 'height', 'hMSL', 'fixType', 'numSV', 'hAcc', 'vAcc', 'roll', \
...
def load_gnss_data(muses_root, scene_meta_data):
    ...
    return read_gnss_file(gnss_path)
...
def points_to_world_coord_ned(pcd_points, ublox_data, sensor2gnss, origin_ublox_data=None):
    """
...
def load_points_in_image_lidar(
    ...
    uv_img_cords_filtered, pcd_points_filtered = filter_and_project_pcd_to_image(
...
def load_lidar_projection(
    lidar_path, calib_data, scene_meta_dict=None, motion_compensation=False,
    ...
    Returns:
        ...
        np.array: The validity mask for the projected lidar points (before enlargement).
    """
    ...
    validity_mask = np.where(image[:, :, 0] != 0, 1, 0).astype(np.uint8)  # Valid points are where the range channel is not zero
